...
class Article extends Base
{
    ...
    public function index()
    {
        ...
        array_shift($years);
        $curArticle = Db::name('article')->where("cat_id != 1 and cat_id != 7 and YEAR(FROM_UNIXTIME(add_time)) = $currentYear")->select();
        foreach($curArticle as &$item){
            $item['content'] = htmlspecialchars_decode($item['content']);
        }
        $history = [];
        foreach($years as $val){
            $info = Db::name('article')->where("cat_id != 1 and cat_id != 7 and YEAR(FROM_UNIXTIME(add_time)) = $val")->select();
