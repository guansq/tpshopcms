 ...
 */
class ProduceCategory extends Validate
{
    //验证规则
    protected $rule = [
    ...
    protected $scene = [
        'add'  => ['name'],
        'edit' => ['name', 'cat_id' => 'require|checkEdit'],
    ...
    protected function checkEmpty($value,$rule, $data)
    {
        ...
        $name_num = M('produce_cat')->where(['name'=>$value])->whereNotIn('cat_id',$data['cat_id'])->count();
        if($name_num > 0){
            return '分类名不能重复';
        }
        return true;
    ...
    protected function checkDel($value)
    {
        ...
        $res = D('produce_cat')->where('parent_id', $value)->select();
        if ($res) {
            return '还有子分类，不能删除';
        }
