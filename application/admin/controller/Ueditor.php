...
class Ueditor extends Base
{
	...
	*/
    ...
    public function videoFileUp()
    {
        ...
        $result = $this->validate(
            ['file' => $file],
            ['file'=>'fileExt:3gp,mp4,rmvb,mov,avi,m4v'],
            ['file.fileExt'=>'上传文件后缀名必须为3gp,mp4,rmvb,mov,avi,m4v']
        );
        if (true !== $result || empty($file)) {
            $state = "ERROR" . $result;
        } else {
            ...
            $info = $file->rule(function ($file) {
                return date('Ymd_').input('name'); // 使用自定义的文件保存规则
            })->move(ROOT_PATH.$path);
            //print_r($info);die;
            if ($info) {
                $state = "SUCCESS";
            ...
            $return_data['url'] = '/public/upload/videos/'.$info->getSaveName();
        }
