...
class ProduceCatLogic extends Model{
    /**
     * 获得指定分类下的子分类的数组
     *
     * @access  public
     ...
     * @return  mix
     */
    public function produce_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
    {
        static $res = NULL;
        ...
        if ($res === NULL)
        {
            ...
            if ($data === false)
            {
                ...
                $sql = "SELECT c.*, COUNT(s.cat_id) AS has_children FROM __PREFIX__produce_cat AS c".
                    " LEFT JOIN __PREFIX__produce_cat AS s ON s.parent_id=c.cat_id $where ".
                    " GROUP BY c.cat_id  ORDER BY parent_id, sort";
                $res = DB::query($sql);
